...
const WorkExperienceForm: React.FC<WorkExperienceFormProps> = ({ index, onDelete, isDisabled }) => {
  ...
  return (
    <div className="work-experience-form">
      ...
      <input type="text" placeholder="Materia Impartida" required disabled={isDisabled} />
      ...
      <select name="nombre_coordinador" id="input-16">
         <option value="">En calidad de:</option>
           {
             infoTitu.map((info) => (
             <option value={info.mod} key={info.mod}>{info.mod}</option>))
               }
       </select>
         ...
         <option value="">Grado Academco que impartio</option>
           {
             ...
             <option value={grad.mod} key={grad.mod}>{grad.mod}</option>))
               }
       </select>
